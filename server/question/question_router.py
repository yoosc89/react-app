...
@router.get('/list', response_model=question_schema.QusetionList)
...
def question_create(_question_cretae: question_schema.QusetionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    question_crud.create_question(
        db=db, question_create=_question_cretae, user=current_user)


@router.put('/update', status_code=status.HTTP_204_NO_CONTENT)
def question_update(_question_update: question_schema.QusetionUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if current_user != db_question.user.id:
        raise HTTPException(
